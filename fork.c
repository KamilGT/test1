...
int main()
{
    pid_t pid; // pid - это идентификатор процесс, его тип pid_t, означает видимо Process ID -  то есть номер прцесса, отличный от других процессов
    ...
    if (pid == 0) // Если это дочерний процесс, то для него pid=0  (pid - это идентификатор процесса_
    { 
    ...
    else // иначе - это не дочерний процесс, значит материнский и у него соответсвенно pid > 0 ( pid - это идентификатор процесса)
    {
